...
class PushTEnv(gym.Env):
    ...
    def render_at_position(self, pos) -> np.ndarray:
        """Render observation with agent at ``pos`` without modifying env state.

        Args:
            pos: 2D position (numpy array or tensor) in 512-space coordinates.
        ...
        if hasattr(pos, "numpy"):
            pos = pos.numpy()
        saved_pos = tuple(self.agent.position)
        self.agent.position = (float(pos[0]), float(pos[1]))
        self.space.reindex_shapes_for_body(self.agent)
        img = self._render_frame("rgb_array")
        self.agent.position = saved_pos
        self.space.reindex_shapes_for_body(self.agent)
        return img
